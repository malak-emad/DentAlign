...
// Import the API function for nurse signup
import { signupNurse } from '../api/authApi';
...
export default function NurseSignup() {
  ...
  // State for nurse-specific fields
  ...
  function validateForm() {
    ...
    // Note: Nursing license is optional for nurses
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const nurseData = {
        ...
        nursing_license_number: nursingLicenseNumber.trim()
      };
      
      console.log('📤 Sending nurse data:', nurseData);
      
      // Call our API
      const result = await signupNurse(nurseData);
      
      if (result.success) {
        // Success! Show custom success message for nurses
        ...
        console.log('✅ Nurse registration successful:', result.data);
      } else {
        // Handle API errors
        ...
        console.error('❌ Nurse registration failed:', result);
      }
    } catch (error) {
  ...
  return (
    <div className={styles.app}>
      {/* Left Panel - Same as other signup forms */}
      ...
      <div className={styles['right-panel']}>
        <div className={styles.card}>
          {/* Nurse-specific Title */}
          <h1 className={styles['card-title']}>Nurse Registration</h1>
          ...
          <form className={styles['login-form']} onSubmit={handleSubmit}>
            ...
            {/* Nursing License Number Field (Optional) */}
            <label className={styles['field-label']}>Nursing License Number (Optional)</label>
            <InputField
              value={nursingLicenseNumber}
              onChange={(v) => setNursingLicenseNumber(v)}
              placeholder="Enter your nursing license number (if available)"
            />
            {errors.nursing_license_number && <span className={styles.error}>{errors.nursing_license_number}</span>}
            ...
            >
              {isLoading ? 'Creating Account...' : 'Register as Nurse'}
            </button>
            ...
            <div className={styles['signup-link']}>
              Doctor registration? <a href="/signup/doctor">Sign up as Doctor!</a>
            </div>
          </form>
        </div>
      </div>
